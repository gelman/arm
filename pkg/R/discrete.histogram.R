discrete.histogram <- function (x, prob, prob2=NULL, prob3=NULL,
  ...
  if (is.numeric (x)) {
    ...
    plot (range(x)+c(-2,2)*bar.width, c(0,max(prob, prob2, prob3)),
    ...
    par (mar=c(3,3,4,1), mgp=c(1.7,.5,0), tck=-.01)
    plot (c(1, n.x.values)+c(-2,2)*bar.width, c(0,max(prob, prob2, prob3)),
        xlab=xlab, ylab=ylab, xaxs="i", xaxt="n",  yaxs="i",
        yaxt=ifelse(is.null(yaxs.label),"s","n"), bty="l", type="n", ...)
  ...
  for (i in 1:length(x)){
    polygon (x[i] + c(-1,-1,1,1)*bar.width/2 + offset[1], c(0,prob[i],prob[i],0),
        border=prob.col, col=prob.col)
    
    if (!is.null(prob2)) {
      polygon (x[i] + c(-1,-1,1,1)*bar.width/2 + offset[2], c(0,prob2[i],prob2[i],0),
          border=prob2.col, col=prob2.col)
    }
    
    if (!is.null(prob3)) {
      polygon (x[i] + c(-1,-1,1,1)*bar.width/2 + offset[3], c(0,prob3[i],prob3[i],0),
          border=prob3.col, col=prob3.col)
    }
  }
