...
#'
#' Bayesian functions for generalized linear modeling with independent
#' normal, t, or Cauchy prior distribution for the coefficients.
#'
#' The program is a simple alteration of \code{glm()} that uses an
#' approximate EM algorithm to update the betas at each step using an
#' augmented regression to represent the prior information.
#'
#' We use Student-t prior distributions for the coefficients. The
#' prior distribution for the constant term is set so it applies to
#' the value when all predictors are set to their mean values.
#'
...
#' one value, we just use prior.scale. For a predictor with two
#' values, we use prior.scale/range(x). For a predictor with more than
#' two values, we use prior.scale/(2*sd(x)).
#'
#' We include all the \code{glm()} arguments but we haven't tested
#' that all the options (e.g., \code{offsets}, \code{contrasts},
#' \code{deviance} for the null model) all work.
#'
#' The new arguments here are: \code{prior.mean}, \code{prior.scale},
#' \code{prior.scale.for.intercept}, \code{prior.df},
#' \code{prior.df.for.intercept}and \code{scaled}.
#' 
...
#'   This should be \code{NULL} or a numeric vector of length either one
#'   or equal to the number of cases. One or more \code{\link{offset}}
#'   terms can be included in the formula instead or as well, and if
#'   both are specified their sum is used.  See
#'   \code{\link{model.offset}}.
#' @param control a list of parameters for controlling the fitting
#'   process. See the documentation for \code{\link{glm.control}} for
#'   details.
...
#'   default method \code{"glm.fit"} uses iteratively reweighted least
...
#' @param drop.unused.levels default TRUE, if FALSE, it interpolates
#'   the intermediate values if the data have integer levels.
#' @param intercept logical. Should an intercept be included in the
#'   \emph{null} model?
#' @param prior.mean prior mean for the coefficients: default is
#'   0. Can be a vector of length equal to the number of predictors (not
#'   counting the intercept, if any). If it is a scalar, it is expanded
#'   to the length of this vector.
#' @param prior.scale prior scale for the coefficients: default is
#'   NULL; if is NULL, for a logit model, prior.scale is 2.5; for a
#'   probit model, prior scale is 2.5*1.6.  Can be a vector of length
#'   equal to the number of predictors (not counting the intercept, if
#'   any). If it is a scalar, it is expanded to the length of this
#'   vector.
#' @param prior.df prior degrees of freedom for the coefficients.  For
#'   t distribution: default is 1 (Cauchy). Set to Inf to get normal
#'   prior distributions. Can be a vector of length equal to the number
#'   of predictors (not counting the intercept, if any).  If it is a
#'   scalar, it is expanded to the length of this vector.
#' @param prior.mean.for.intercept prior mean for the intercept:
#'   default is 0. See \sQuote{Details}.
#' @param prior.scale.for.intercept prior scale for the intercept:
#'   default is NULL; for a logit model, prior scale for intercept is
#'   10; for probit model, prior scale for intercept is rescaled as
#'   10*1.6.
...
#'   default is 1e-12.
#' @param scaled scaled=TRUE, the scales for the prior distributions
#'   of the coefficients are determined as follows: For a predictor with
#'   only one value, we just use prior.scale. For a predictor with two
#'   values, we use prior.scale/range(x). For a predictor with more than
#'   two values, we use prior.scale/(2*sd(x)). If the response is
#'   Gaussian, prior.scale is also multiplied by 2 * sd(y). Default is
#'   TRUE.
#' @param keep.order a logical value indicating whether the terms
#'   should keep their positions. If \code{FALSE} the terms are
#'   reordered so that main effects come first, followed by the
#'   interactions, all second-order, all third-order and so on.  Effects
#'   of a given order are kept in the order specified. Default is TRUE.
#' @param drop.baseline Drop the base level of categorical x's,
#'   default is TRUE.
#' @param n.iter default is 100.
#' @param print.unnormalized.log.posterior display the unnormalized
#'   log posterior likelihood for bayesglm, default=FALSE.
...
#'   \item{prior.df}{prior dfs for the coefficients.}
#'   \item{prior.scale.for.intercept}{prior scale for the intercept}
#'   \item{prior.df.for.intercept}{prior df for the intercept}
...
#' @seealso \code{\link[stats]{glm}}, \code{\link{bayespolr}}
  ...
  print.unnormalized.log.posterior = FALSE, Warning=TRUE,...)
{
  ...
  # 2013.8.21  I input data instead of mf here.  Don't know if this is right
  ...
  fit$null.deviance <- bayesglm.fit(x = X[, "(Intercept)", drop = FALSE], y = Y, weights = weights,
            ...
            prior.scale.for.intercept = prior.scale.for.intercept,
            prior.df.for.intercept = prior.df.for.intercept,
            min.prior.scale = min.prior.scale, scaled = scaled, Warning=Warning)$deviance
  }
